...
public class Record : IRecord
{
    ...
    public delegate IRecordValue RecordValueFactoryDelegate(ReadOnlySpan<byte> data, bool isBigEndian);
    private readonly RecordValueFactoryDelegate? m_ValueFactoryDelegate;

    public IRecordValue RecordValueFactory(ReadOnlySpan<byte> data, bool isBigEndian) => m_ValueFactoryDelegate?.Invoke(data, isBigEndian) ?? throw new Exception("no record value");

    public Record(int tag, RecordType type, RecordLength len, Dictionary<IRecordValue, string> map)
    {
        Tag = tag;
        RecordType = type;
        Length = len;
    ...
    public Record(int tag, RecordType type, RecordLength len, RecordValueFactoryDelegate valueFactory)
    {
        RecordType = type;
        Length = len;
        Tag = tag;
    ...
    public IRecordValue? Parse(ReadOnlySpan<byte> data, ref long position, bool isBigEndian)
    {
        ...
        if (position + length > data.Length)
        {
            ...
            return new JpError("Directory truncated", isBigEndian);
        }

        Data = new byte[length];

        data.Slice((int)position, length).CopyTo(Data);
        position += length;
        ...
        if (MapValueDescription != null)
        {
            ...
            foreach (KeyValuePair<IRecordValue, string> kv in MapValueDescription)
            {
                ...
                {
                    return new RecordValues.JpConstant(kv.Value, isBigEndian);
                }
            }
        }
        else
        {
            IRecordValue value = RecordValueFactory(Data, isBigEndian);
